<template>
  <div class="trust-picker-select-item">
    <div class="trust-picker-select-line"/>
    <div class="trust-picker-select-list">
      <ul
        class="trust-picker-select-ul"
        ref="list">
        <li
          class="trust-picker-select-list-item"
    ...
    <ul
      class="trust-picker-select-wheel"
      ref="wheel">
      <li
        class="trust-picker-select-wheel-item"
        :class="{'hidden':setHidden(el.index)}"
  ...
  export default {
    ...
    methods: {
      ...
      setStyle (move, type, time) {
        ...
        let spinAim = Math.floor(updateDeg / 20)
        let margin = Math.floor(updateMove / singleHeight) * singleHeight // 如果不这么写 会导致没有滚动效果
        /* 计算touchEnd移动的整数距离 */
        let endMove = margin
        let endDeg = Math.floor(updateDeg / deg) * deg
        if (type === 'end') {
